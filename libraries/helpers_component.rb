...
module Cloud
    ...
    module Component
      ...
      def component_node(node, component)
        Chef::Node.json_create(
          Chef::ServerAPI.new(
            Cloud::Helpers::ChefServer.chef_server_config(node)[:chef_server_url],
            client_name: Cloud::Helpers::ChefServer.chef_server_config(node)[:options][:client_name],
            signing_key_filename: Cloud::Helpers::ChefServer.chef_server_config(node)[:options][:signing_key_filename]
      ...
      def component_fqdn(node, component, c_node = component_node(node, component))
        node['cloud-provisioning'][component]['fqdn'] ||
          node['cloud-provisioning'][component]['host'] ||
          Cloud::Helpers.get_ip(node, c_node)
      end
      ...
      def component_hostname(node, component, id = nil)
        Cloud::Helpers.check_attribute?(
          node['cloud-provisioning'][component],
          "node['cloud-provisioning']['#{component}']"
        )
        if id
      ...
      def single_component_hostname(node, component)
        unless hostname?(get_component(node, component))
          ...
          node.set['cloud-provisioning'][component]['hostname'] = "#{component_prefix}-#{Cloud::Helpers.chef_provisioning_id(node)}"
        end
      ...
      def multiple_component_hostname(node, component, id)
        unless hostname?(get_component(node, component, id))
          unless node['cloud-provisioning'][component]['hostname_prefix']
            node.set['cloud-provisioning'][component]['hostname_prefix'] = "node-#{Cloud::Helpers.chef_provisioning_id(node)}"
          end
          node.set['cloud-provisioning'][component][id]['hostname'] = "#{node['cloud-provisioning'][component]['hostname_prefix']}-#{id}"
        end
      ...
      def get_component(node, name, id = nil)
        if id
          return {} unless node['cloud-provisioning'][name][id]
          node['cloud-provisioning'][name][id]
        else
          node['cloud-provisioning'][name]
        end
      end
      ...
      def component_attributes(node, name)
        node['cloud-provisioning'][name]['attributes']
      rescue
        {}
      end
    end
  ...
  module DSL
    ...
    def component_attributes(component)
      Cloud::Helpers::Component.component_attributes(node, component)
    end

    # The component node object
    def component_node(component)
      Cloud::Helpers::Component.component_node(node, component)
    end

    # The component fqdn
    def component_fqdn(component, component_node = nil)
      Cloud::Helpers::Component.component_fqdn(node, component, component_node)
    end

    # The component hostname
    def component_hostname(component, id = nil)
      Cloud::Helpers::Component.component_hostname(node, component, id)
    end
  end
end
